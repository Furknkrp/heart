...
function createFloatingHearts() {
    ...
    if (!container) return;
    container.innerHTML = '';
    const waveCount = 3, heartsPerWave = 25;
    const heartColors = ['#ff7b9b', '#ffacc5', '#f34f85', '#ffcad4', '#e0218a'];
    const heartPath = "M0.5,1 C0.5,1,0,0.7,0,0.3 A0.25,0.25,0,0,1,0.5,0.3 A0.25,0.25,0,0,1,1,0.3 C1,0.7,0.5,1,0.5,1 Z";
    for (let j = 0; j < waveCount; j++) {
        const wave = document.createElement('div');
        wave.classList.add('heart-wave');
        wave.style.animationDelay = `${j * 4}s`;
        for (let i = 0; i < heartsPerWave; i++) {
            const heart = document.createElement('div');
            heart.classList.add('heart');
            const randomColor = heartColors[Math.floor(Math.random() * heartColors.length)];
            heart.innerHTML = `<svg viewBox="0 0 1 1" width="100%" height="100%" preserveAspectRatio="xMidYMid meet"><path d="${heartPath}" fill="${randomColor}"></path></svg>`;
            const size = Math.random() * 35 + 20;
            const top = Math.random() * 80 + 10, left = Math.random() * 90 + 5;
            heart.style.width = `${size}px`;
            heart.style.height = `${size}px`;
            heart.style.top = `${top}%`;
            heart.style.left = `${left}%`;
            heart.style.animationDelay = `${Math.random() * 3}s`;
            wave.appendChild(heart);
        }
...
function Heart() {
    ...
    this.size = Math.random() * 25 + 15;
    let baseSpeed = (Math.random() * 1.0 + 0.5);
...
function handleInteraction(event) {
    ...
    for (let i = hearts.length - 1; i >= 0; i--) {
        ...
        if (d < h.size) {
            ...
            score += pointsToAdd; scoreValueEl.textContent = score; syncScoreToFirebase();
            ...
            if (score > 0 && score % 7 === 0) { speedModifier += 0.07; spawnRateModifier *= 0.97; currentSpawnRate = baseSpawnRate * spawnRateModifier; }
...
function init() {
    ...
    // Mobil ve Masaüstü için basılı tutma olayları
    turboBtn.addEventListener('mousedown', startTurbo);
    turboBtn.addEventListener('mouseup', stopTurbo);
    turboBtn.addEventListener('touchstart', (e) => { e.preventDefault(); startTurbo(); });
    turboBtn.addEventListener('touchend', stopTurbo);
}
